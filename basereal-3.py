...
class BaseReal:
    ...
    def add_completion_callback(self, callback):  
        self._completion_callbacks = getattr(self, '_completion_callbacks', [])  
        self._completion_callbacks.append(callback)  
      
    def execute_completion_callbacks(self):  
    ...
    # 如果这是表演确认消息，设置一个回调在消息播放完成后执行    
        if eventpoint:  
            if eventpoint.get('type') == 'performance_confirmation':    
            # 设置在消息结束后自动转换到表演状态    
                self.tts.add_completion_callback(lambda: self.set_curr_state(2, True))  
            elif eventpoint.get('type') == 'conversation_end' and eventpoint.get('maintain_state') is not None:  
            # 设置在对话结束后恢复之前的状态  
                self.tts.add_completion_callback(lambda: self.set_curr_state(eventpoint.get('maintain_state'), False))
    ...
    #     # 修改这里：不自动切换到静态状态，而是重置索引以循环播放  
    #         self.custom_audio_index[audiotype] = 0  
    #     # 可选：添加事件通知  
    #         self.notify({"type": "audio_cycle_complete", "audiotype": audiotype})  
    #     return stream
    ...
    def get_audio_stream(self, audiotype):
        ...
        # 表演音频播放完毕，切换回静态状态
            self.custom_audio_index[audiotype] = 0
        if audiotype == 2:  # 2为表演状态
            self.set_curr_state(1, True)  # 切回静态
            self.notify({"type": "performance_end"})
        return stream
