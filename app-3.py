...
def llm_response(message, nerfreal):  
    # 存储当前状态，确保在处理完毕后可以恢复  
    current_state = nerfreal.curr_state  
    ...
    if "表演" in message and ("古筝" in message or "节目" in message):    
        ...
        # 返回表演开始的提示信息    
        nerfreal.put_msg_txt("好的爸爸，我这就为您表演一段古筝曲。", {"type": "performance_confirmation"})    
        return  
            ...
            for i, char in enumerate(msg):  
                if char in ",.!;:，。！？：；" :  
                    ...
                    if len(result)>10:  
                        print(result)  
                        # 传递事件点以保持状态  
                        nerfreal.put_msg_txt(result, {"type": "conversation", "maintain_state": current_state})  
                        result=""  
            result = result+msg[lastpos:]  
    ...
    print(f"llm Time to last chunk: {end-start}s")  
    # 最后一段文本也需要传递事件点  
    nerfreal.put_msg_txt(result, {"type": "conversation_end", "maintain_state": current_state})
...
async def get_audiotype(request):
    params = await request.json()
    sessionid = params.get('sessionid', 0)
    audiotype = nerfreals[sessionid].curr_state if sessionid in nerfreals else 1
    return web.Response(
        content_type="application/json",
        text=json.dumps({"code": 0, "audiotype": audiotype}),
    )
...
if __name__ == '__main__':
    ...
    appasync.router.add_post("/get_audiotype", get_audiotype)
